...
plot_climate_comparison <- function(climate_data = NULL,
  ...
  if (rolling_mean) {
    climate_data <- climate_data %>%
      ...
      dplyr::arrange(date) %>%
      dplyr::mutate(
        moving_avg_precip = zoo::rollmean(daily_sum_precip, k = 5, fill = NA, align = "center"),
  ...
  past <- climate_data %>%
    ...
    dplyr::mutate(
      upper = my_max(!!variable, na.rm = TRUE), # identify max value for each day
      ...
      se = sd(!!variable, na.rm = TRUE) / sqrt(length(!!variable))
    ) %>% # calculate standard error of mean
    dplyr::mutate(
      avg_upper = avg + (qt(1 - .05 / 2, nrow(.)) * se), # calculate 95% CI for mean (get the appropriate t-value for the number of observations)
      avg_lower = avg - (qt(1 - .05 / 2, nrow(.)) * se)
  ...
  if (annotate_plot) {
    if (no_high_days > 0) {
      if (high_annot_coord[[1]] > 250) {
        p <- p +
          annotate("segment",
            x = high_annot_coord[[1]], xend = high_annot_coord[[1]] - 40,
            y = high_annot_coord[[2]], yend = high_annot_coord[[2]] + 5, colour = "firebrick3"
          ) +
          annotate("text",
            x = high_annot_coord[[1]] - 140, y = high_annot_coord[[2]] + 4,
            label = paste0(
              no_high_days,
        ...
        p <- p +
          annotate("segment",
            x = high_annot_coord[[1]], xend = high_annot_coord[[1]] + 10,
            y = high_annot_coord[[2]], yend = high_annot_coord[[2]] + 5, colour = "firebrick3"
          ) +
          annotate("text",
            x = high_annot_coord[[1]] + 12, y = high_annot_coord[[2]] + 4,
            label = paste0(
              no_high_days,
              text_table[[language]][4],
              text_table[[language]][5]
  ...
  if (annotate_plot) {
    ...
    if (no_low_days > 0) {
      if (low_annot_coord[[1]] > 250) {
        p <- p +
          annotate("segment",
            x = low_annot_coord[[1]], xend = low_annot_coord[[1]] - 40,
            y = low_annot_coord[[2]], yend = low_annot_coord[[2]] - 5, colour = "blue3"
          ) +
          annotate("text",
            x = low_annot_coord[[1]] - 70, y = low_annot_coord[[2]] - 12,
            label = paste0(
              no_low_days,
        ...
        if (low_annot_coord[[1]] <= 250 & low_annot_coord[[1]] > 150) {
          p <- p +
            ...
            annotate("text",
              x = low_annot_coord[[1]] - 38, y = low_annot_coord[[2]] - 22,
              label = paste0(
                no_low_days,
                text_table[[language]][6],
                text_table[[language]][7]
          ...
          p <- p +
            annotate("segment",
              x = low_annot_coord[[1]], xend = low_annot_coord[[1]] + 10,
              y = low_annot_coord[[2]], yend = low_annot_coord[[2]] - 5, colour = "blue3"
            ) +
            annotate("text",
              x = low_annot_coord[[1]] + 12, y = low_annot_coord[[2]] - 10,
              label = paste0(
                no_low_days,
                text_table[[language]][6],
                text_table[[language]][7]
