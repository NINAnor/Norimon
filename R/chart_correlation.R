...
chart_correlation <- function(R,
  ...
  panel.cor <- function(x,
    ...
    test <- stats::cor.test(as.numeric(x), as.numeric(y), method = method)
    Signif <- stats::symnum(test$p.value,
      corr = FALSE, na = FALSE,
    ...
    graphics::text(0.5, 0.5, txt, cex = cex * (abs(r) + 0.3) / 1.3)
    graphics::text(0.8, 0.8, Signif, cex = cex, ..., col = color)
  }
  # f <- function(t) {
  ...
  hist.panel <- function(x, ... = NULL) {
    ...
    graphics::lines(stats::density(x, na.rm = TRUE), lwd = 1, col = color)
    graphics::rug(x)
  }
  if (histogram) {
