...
# Introduction
I will here introduce some functions that can be used to produce insect indicators from the Norimon package. Their use isn't limited to indicators, essentially they are meant to be of general use, to fetch and plot quantities from the insect monitoring program. 
...
1. Fetch observations of specific species or higher taxa from a centralized database (or GBIF)
...
Most of the ecology comes into step 1, in choosing what data to use to describe a quality. This can be a custom selection of species, or a set of higher taxa. The goal is to choose a set of taxa that represent specific qualities of the community. These qualities will indicate the ecological quality of the ecosystem. We can also decide on the level of aggregation, e.g. if we should aggregate the data on the region scale, locality scale, or even the individual sample occation scale. Ecology also comes into step 3, comparing the values to a reference state. Here we have several options. We could for example use a single defined value as the reference state. But we can also use a point in time as a reference point (e.g. the start of a time series).
...
ind_tab <- tibble("Indicator" = c("Total biomass",
                  ...
                  "Referencing" = c("boot_contrast()",
                                    "boot_contrast()",
                                    "- expert opinion reference point",
                                    "- expert opinion reference point"))
...
Let's say that we have an expert opinion based reference value of the Shannon diversity of pollinators of 4. We can then subtract this value to get the difference, and thus the relative value.
